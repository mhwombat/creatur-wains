...
hypothesise
  :: Word32 -> [[(Label, UI.Double)]] -> [([Label], UI.Double)]
...
jointProbability :: [(Label, UI.Double)] -> ([Label], UI.Double)
...
diffsToProbs :: Word32 -> [[(Label, UI.Double)]] -> [[(Label, UI.Double)]]
diffsToProbs x = map (diffsToProbs2 x)

-- | Given the signature for one input pattern, calculate the
--   probability that the pattern should match each classifier model.
diffsToProbs2 :: Word32 -> [(Label, UI.Double)] -> [(Label, UI.Double)]
...
diffsToProbs1 :: Word32 -> [UI.Double] -> [UI.Double]
